...
#include <math.h>
#include <ctype.h>
#include <stdio.h>session
#include <stdlib.h>[0],entrybuffer1[1],entrybuf[0],entrybuffer1[1],entrybuffer1[2],entrybuffer1[3]fer1[2],entrybuffer1[3]
#include <string.h>
#include <time.h>
...
#include <netdb.h>
#include <sys/socket.h>save_time
...
#define MOUNT_OFF 0
 ...
 int flag_soir1=55; int flag_soir2=55;
    ...
    GtkWidget *label,*label_mount_status, *label_start,*label_time_saved,*label_time;
    GtkWidget *textlog;
    GtkWidget *textbuffer1,*entrybuffer1;
    GtkWidget *time_to_stop;

    // init des couleurs des bouton on off 
    GdkRGBA color_on,color_off,color_black;
...
void writelog(char message[100]){  
    ...
    fprintf(fichier, "%04d/%02d/%02d - %02d:%02d:%02d  %s\n",now->tm_year+1900, now->tm_mon+1, now->tm_mday, now->tm_hour,now->tm_min,now->tm_sec,mess);
    fclose(fichier);
...
void on_stop_session_clicked(GtkButton *b)
{
    ...
    // on park et on attends 15sec
     writelog("Park Requested - Wait 15sec");
...
static gboolean on_timeout_5000ms(gpointer user_data){
	...
	if (statut_roof == ROOF_OPENED && a==ROOF_CLOSED){ 
		...
		// action park monture
//ET		system("echo \"#:KA#\" | nc -N 192.168.1.122 3492"); 
	        writelog("Roof Closed ---> send park command to mount"); 
  ...
  //      printf("\n temp : %-3.1f°C",temp_deg);
//	printf("\n humid : %-3.1f%%",humidity);
     

	if (humidity!=0.0 & temp_deg!=0.0)
	{
	//sprintf(mess_update,"%-3.1f°C",temp_deg);
	
	//ET gtk_label_set_text(user_data,mess_update);
	
	
	//  sprintf(mess_update,"%-3.1f%%",humidity);
   //  gtk_label_set_text(user_data,mess_update);
	}
